...
def get_sidebar_items_for_user(user):
    ...
    visible_sidebar = []
    for category in _registered_sidebar_items:
        if category.is_role_allowed(user_role):
            visible_items = [item for item in category.items if item.is_role_allowed(user_role)]
            if visible_items:
                visible_sidebar.append(DashboardCategory(name=category.name, roles=category.roles, items=visible_items))
        else:
            continue
